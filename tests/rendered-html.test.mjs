...
test("exports the geometric sakura theme and accessible decorative system", () => {
  ...
  assert.match(homepage, /translate\(113 238\) rotate\(-8\) scale\(0\.72\)/);
  assert.match(homepage, /translate\(218 202\) rotate\(7\) scale\(0\.9\)/);
  assert.match(homepage, /translate\(373 119\) rotate\(-4\) scale\(0\.82\)/);
  assert.match(homepage, /translate\(518 96\) rotate\(11\) scale\(0\.68\)/);
  assert.doesNotMatch(homepage, /hero-garden-grid|k3-mark-fill|falling-petals/);
  ...
  assert.match(css, /\.hero-garden-branch path\{[^}]*stroke-width:1\.25px/);
  assert.match(css, /\.hero-garden-mountains path\{[^}]*stroke:rgba\(75,38,125,.1\)/);
  assert.match(css, /\.hero-garden-signature path\{[^}]*stroke-width:1\.5px/);

...
export default async function Demo({params}:{params:Promise<{slug:string}>}){const{slug}=await params;const c=concepts[slug as keyof typeof concepts];if(!c)notFound();const signature=c.name.charAt(0);return <main className={`concept ${c.tone}`}><div className="concept-note">CONCEPT / DEMO · MOCK CONTENT <Link href="/websites">← Return to website examples</Link></div><nav className="concept-nav"><strong>{c.name}</strong><div>{c.nav.map(x=><a href="#details" key={x}>{x}</a>)}</div><a className="concept-action" href="#contact">{c.action}</a></nav><header className="concept-hero"><span>{c.label}</span><h1>{c.strap}</h1><p>{c.detail}</p><a href="#details">{c.action} ↘</a><div className="concept-art"><b data-text={signature}>{signature}</b></div></header><section className="concept-details" id="details"><span>SELECTED / 01—03</span><h2>Made to feel specific,<br/>built to stay flexible.</h2><div className="concept-cards">{c.cards.map((x,i)=><article key={x}><small>0{i+1}</small><div className="card-art"><b>{x.charAt(0)}</b></div><h3>{x}</h3><p>Thoughtful details and clear information, ready to replace with the real business content.</p></article>)}</div></section><section className="concept-contact" id="contact"><p>OPENING HOURS · LOCATION · CONTACT · MOCK DETAILS</p><h2>Come and find us.</h2><Link href="/services">Discuss your website project ↗</Link></section></main>}
